...
# 文件说明
    ...
    本地证书 traefik 的docker
    docker-compose.local-certs.yml

    自动证书 traefik 的docker
...
# 购买或自签名 HTTPS 证书
    ...
    * 拷贝
    cp docker-compose.local-certs.yml docker-compose-local.yml
    * 启动
...
# 启动 Traefik 并自动申请 HTTPS 证书(本例子)

    * 拷贝
    cp docker-compose.acme.yml docker-compose-acme.yml
    * 启动
    docker compose down && docker compose up -d
